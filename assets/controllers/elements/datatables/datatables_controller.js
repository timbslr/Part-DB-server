...
export default class extends Controller {
    ...
    connect() {
        ...
        const GROUPING_ENABLED = window.location.pathname.endsWith(
            "/parts-by-storage-location",
        );
        ...
        if (saved_state !== null) {
            ...
            settings.initial_order = settings.initial_order.filter(
                (order) => order !== null,
            );
        ...
        promise.then((dt) => {
            ...
            const lengthSelectors = document.querySelectorAll(
                'label select[name="dt_length"]',
            );
        ...
        promise.then((dt) => {
            dt.on("select deselect", () => {
                $(".group-select-checkbox").each(function () {
                    const $checkbox = $(this);
                    const groupName = $checkbox.closest("tr").data("group");
                    const rowsInGroup = dt.rows(
                        (idx, data) => data.storelocation === groupName,
                    );

                    const selectedCount = rowsInGroup
                        .nodes()
                        .to$()
    ...
    updateAmountHeaderWithTotalAmount() {
        ...
        const total = dt
            ...
            .reduce(
                (sum, val) =>
                    isNaN(parseFloat(val)) ? sum : sum + parseFloat(val),
                0,
            );
