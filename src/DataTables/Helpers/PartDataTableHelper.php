...
class PartDataTableHelper
{
    ...
    public function renderName(Part $context): string
    {
        ...
        if ($context->isFavorite()) {
            $icon = sprintf(
                '<i class="fa-solid fa-star fa-fw me-1" title="%s"></i>',
                $this->translator->trans('part.favorite.badge')
            );
        }
        if ($context->isNeedsReview()) {
            $icon = sprintf(
                '<i class="fa-solid fa-ambulance fa-fw me-1" title="%s"></i>',
                $this->translator->trans('part.needs_review.badge')
            );
        }
        if ($context->getBuiltProject() instanceof Project) {
            $icon = sprintf(
                '<i class="fa-solid fa-box-archive fa-fw me-1" title="%s"></i>',
    ...
    public function renderAmount(Part $context): string
    {
        ...
        if ($context->isAmountUnknown()) {
            ...
            if ($amount === 0.0) {
                $ret .= sprintf(
                    '<b class="text-primary" title="%s">?</b>',
        ...
        if ($expiredAmount > 0) {
            $ret .= sprintf(
                ' <span title="%s" class="text-muted">(+%s)</span>',
        ...
        if ($context->isNotEnoughInstock()) {
            $ret = sprintf(
                '<b class="text-danger" title="%s">%s</b>',
